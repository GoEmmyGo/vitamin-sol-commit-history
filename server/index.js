...
const {getWeather} = require('./APIcontroller.js')

// So if we want to be able to accept json in any other requests that come to the server need to put in this line, so that any json that comes in will be parsed and then translated into javascript which we can use, so we have to have this line if we want to accept post request input request specifically.
